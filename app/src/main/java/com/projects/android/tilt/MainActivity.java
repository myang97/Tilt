...
public class MainActivity extends Activity implements SensorEventListener {
    ...
    public void onSensorChanged(SensorEvent event) {
        if (event.sensor.getType() == Sensor.TYPE_ACCELEROMETER) {
            ...
            for(int i = 0; i < ballArray.length; i++){
                ...
                if(level1 && !level2 && ballArray[i].getInHole()){
                    inHoleCounter++;
                }
            }
            if(!level2 && inHoleCounter == 5){
                level2 = true;
            }
        }
    }
    ...
    public class AnimatedView extends ImageView {
        ...
        protected void onDraw(Canvas canvas) {
            ...
            canvas.drawText(" Level 1", phoneWidth / 3, phoneHeight / 3 + 420, paint);
            //canvas.drawText("CONGRATULATIONS!", phoneWidth / 12 - 10, phoneHeight / 3, paint);
            //canvas.drawText("YOU BEAT THE GAME!", phoneWidth / 12 - 20, phoneHeight / 3 + 140, paint);
